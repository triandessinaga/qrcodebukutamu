...
function registerUser($email, $password, $userRole) {
    global $conn;
    ...
    $stmt = $conn->prepare("INSERT INTO user (user_email, user_password, user_role) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $email, $hashedPassword, $userRole);
    $stmt->execute();
    $stmt->close();
}
